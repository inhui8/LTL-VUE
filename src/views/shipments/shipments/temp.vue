<template>
  ...
  const showQuoteDetails = (row) => {
    ...
    currentRow.value = row; // Store the reference to the current row
    selectedQuote.value = null; // Reset the selected quote
  ...
  const calculateRecommendedPrice = (row) => {
    ...
    // Filter out invalid prices and find the lowest one
      ...
      if (response && response.data && Array.isArray(response.data)) {
        ...
        while (index1 < tableData.value.length && index2 < response.data.length) {
          ...
          if (tableSoNumber === responseSoNumber) {
            ...
            const xpoPriceResponse = responseItem.XPOResponse;
            if (xpoPriceResponse && xpoPriceResponse.startsWith("Error")) {
              ElMessage({
                message: `XPO API 错误: ${xpoPriceResponse}`,
                type: 'error',
                duration: 5000
              });
              row.xpo_price = 'N/A';
            } else if (xpoPriceResponse) {
              row.xpo_price = xpoPriceResponse;
            } else {
              row.xpo_price = 'N/A';
            }
